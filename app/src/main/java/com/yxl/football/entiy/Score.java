package com.yxl.football.entiy;

/**
 * Created by Administrator on 2018/5/31 0031.
 */

public class Score {

    private String fid;
    private String lid;
    private String lname;
    private String stime;
    private String w;
    private String p;
    private String l;
    private String pshow;
    private String remark;
    private String vsdate;
    private String hname;
    private String hsxname;
    private String hscore;
    private String hhalfscore;
    private String hredcard;
    private String ordernum;
    private String aname;
    private String asxname;
    private String ascore;
    private String ahalfscore;
    private String aredcard;
    private String status;
    private String zl;
    private String ismain;
    private String isjczq;
    private String iswdls;
    private String extrastatusid;
    private String extrascore;
    private String halfstarttime;
    private String isfollow;
    private String sfcordernum;
    private String dcordernum;
    private String hstanding;
    private String astanding;
    private String recom;

    public String getFid() {
        return fid;
    }

    public void setFid(String fid) {
        this.fid = fid;
    }

    public String getLid() {
        return lid;
    }

    public void setLid(String lid) {
        this.lid = lid;
    }

    public String getLname() {
        return lname;
    }

    public void setLname(String lname) {
        this.lname = lname;
    }

    public String getStime() {
        return stime;
    }

    public void setStime(String stime) {
        this.stime = stime;
    }

    public String getW() {
        return w;
    }

    public void setW(String w) {
        this.w = w;
    }

    public String getP() {
        return p;
    }

    public void setP(String p) {
        this.p = p;
    }

    public String getL() {
        return l;
    }

    public void setL(String l) {
        this.l = l;
    }

    public String getPshow() {
        return pshow;
    }

    public void setPshow(String pshow) {
        this.pshow = pshow;
    }

    public String getRemark() {
        return remark;
    }

    public void setRemark(String remark) {
        this.remark = remark;
    }

    public String getVsdate() {
        return vsdate;
    }

    public void setVsdate(String vsdate) {
        this.vsdate = vsdate;
    }

    public String getHname() {
        return hname;
    }

    public void setHname(String hname) {
        this.hname = hname;
    }

    public String getHsxname() {
        return hsxname;
    }

    public void setHsxname(String hsxname) {
        this.hsxname = hsxname;
    }

    public String getHscore() {
        return hscore;
    }

    public void setHscore(String hscore) {
        this.hscore = hscore;
    }

    public String getHhalfscore() {
        return hhalfscore;
    }

    public void setHhalfscore(String hhalfscore) {
        this.hhalfscore = hhalfscore;
    }

    public String getHredcard() {
        return hredcard;
    }

    public void setHredcard(String hredcard) {
        this.hredcard = hredcard;
    }

    public String getOrdernum() {
        return ordernum;
    }

    public void setOrdernum(String ordernum) {
        this.ordernum = ordernum;
    }

    public String getAname() {
        return aname;
    }

    public void setAname(String aname) {
        this.aname = aname;
    }

    public String getAsxname() {
        return asxname;
    }

    public void setAsxname(String asxname) {
        this.asxname = asxname;
    }

    public String getAscore() {
        return ascore;
    }

    public void setAscore(String ascore) {
        this.ascore = ascore;
    }

    public String getAhalfscore() {
        return ahalfscore;
    }

    public void setAhalfscore(String ahalfscore) {
        this.ahalfscore = ahalfscore;
    }

    public String getAredcard() {
        return aredcard;
    }

    public void setAredcard(String aredcard) {
        this.aredcard = aredcard;
    }

    public String getStatus() {
        return status;
    }

    public void setStatus(String status) {
        this.status = status;
    }

    public String getZl() {
        return zl;
    }

    public void setZl(String zl) {
        this.zl = zl;
    }

    public String getIsmain() {
        return ismain;
    }

    public void setIsmain(String ismain) {
        this.ismain = ismain;
    }

    public String getIsjczq() {
        return isjczq;
    }

    public void setIsjczq(String isjczq) {
        this.isjczq = isjczq;
    }

    public String getIswdls() {
        return iswdls;
    }

    public void setIswdls(String iswdls) {
        this.iswdls = iswdls;
    }

    public String getExtrastatusid() {
        return extrastatusid;
    }

    public void setExtrastatusid(String extrastatusid) {
        this.extrastatusid = extrastatusid;
    }

    public String getExtrascore() {
        return extrascore;
    }

    public void setExtrascore(String extrascore) {
        this.extrascore = extrascore;
    }

    public String getHalfstarttime() {
        return halfstarttime;
    }

    public void setHalfstarttime(String halfstarttime) {
        this.halfstarttime = halfstarttime;
    }

    public String getIsfollow() {
        return isfollow;
    }

    public void setIsfollow(String isfollow) {
        this.isfollow = isfollow;
    }

    public String getSfcordernum() {
        return sfcordernum;
    }

    public void setSfcordernum(String sfcordernum) {
        this.sfcordernum = sfcordernum;
    }

    public String getDcordernum() {
        return dcordernum;
    }

    public void setDcordernum(String dcordernum) {
        this.dcordernum = dcordernum;
    }

    public String getHstanding() {
        return hstanding;
    }

    public void setHstanding(String hstanding) {
        this.hstanding = hstanding;
    }

    public String getAstanding() {
        return astanding;
    }

    public void setAstanding(String astanding) {
        this.astanding = astanding;
    }

    public String getRecom() {
        return recom;
    }

    public void setRecom(String recom) {
        this.recom = recom;
    }
}
